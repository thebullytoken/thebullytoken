/** 
    ...
    This project was launched to create an opportunity for everyone
...
contract Bully is Context, IERC20, Ownable {
    ...
    address private _donationAddress = 0x55Ed7BC8275C26f1FF475D94094753088a4517Fa;

    string private _name = "The Bully Token";
    ...
    function deliver(uint256 tAmount) public {
        ...
        (,uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tDonation) = _getTValues(tAmount);
        
        (uint256 rAmount,,) = _getRValues(tAmount, tFee, tLiquidity, tBurn, tDonation, _getRate());
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        (,uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tDonation) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount,) = _getRValues(tAmount, tFee, tLiquidity, tBurn, tDonation, _getRate());
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tDonation) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tBurn, tDonation, _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity);
                tTransferAmount = tTransferAmount.sub(tBurn).sub(tDonation);

        return (tTransferAmount, tFee, tLiquidity, tBurn, tDonation);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tDonation, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rDonation = tDonation.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rBurn).sub(rDonation);
